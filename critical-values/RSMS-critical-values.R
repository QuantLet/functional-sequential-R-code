##########################################################################################
# Date: Correct Parallel Version (First Method Only, Range-Adjusted, for d = 1 to 10)
##########################################################################################
...
numrep <- 1000               # number of repetitions
d_vec <- 9:10 # Dimensions to compute
...
results_list <- foreach(i = 1:numrep, .combine = "rbind", 
                        .options.snow = opts,
                        .export = c("m.chan", "T.chan", "gamma")) %dopar% {
                          ...
                          for (d_index in seq_along(d_vec)) {
                            ...
                            if (d==1) { 
                              temp_mat <-as.numeric(temp_mat)
                               R <-  (max(temp_mat) - min(temp_mat))^(-2)
                            } else {   R <- diag((apply(temp_mat, 2, max) - apply(temp_mat, 2, min))^(-2)) }
                            
                            
                            
                            # Compute statistic
                            if (d == 1) {
                              result_row[d_index] <- max((U.2.s^2 * R) / ((1 + s)^2 * (s/(1+s))^(2*gamma)))
                            } else {
                              result_row[d_index] <- max((t(U.2.s) %*% R %*% U.2.s) / ((1 + s)^2 * (s / (1 + s))^(2 * gamma)))
...
for (d_index in seq_along(d_vec)) {
  d <- d_vec[d_index]
  temp_mat.file.name <- paste0("Range_parallel_T_", T.chan, "_gamma_", gamma, "_d", d, "_nrep_", numrep, ".csv")
